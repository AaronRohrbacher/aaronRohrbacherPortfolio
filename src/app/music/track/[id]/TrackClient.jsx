...
export default function TrackClient() {
  ...
  async function fetchTrack() {
    ...
    try {
      ...
      const url = `/api/track?id=${encodeURIComponent(id)}${shareToken ? `&share=${encodeURIComponent(shareToken)}` : ''}`;
  ...
  function getDownloadUrl(format) {
    ...
    return `/api/stream?id=${encodeURIComponent(id)}&format=${format}&download=1${share}`;
  }
